...
public class AdministratorClient {
	...
	// administrator (determined by the IPAddress) attempts to, if the credentials
	// are
	// valid, concurrently find out the number of online players and offline players
	// in the
	// other geo-locations using UDP/IP sockets and returns the result to the
	// administrator. Please note that it only returns the player counts (a number)
	// and not
	// the player information. For example, if NA has 6 players online, EU has 7
	// players
	// online and AS has 8 players online and they each have 1 player offline it
	// should
	// return the following (which should also be stored in the server and
	// administrator
	// log): NA: 6 online, 1 offline. EU: 7 online, 1 offline, AS: 8 online, 1
	// offline.
	// Thus, this application has a number of GameServers (one per geo-location)
	// each
	// implementing the above operations for that geo-location, PlayersClient (one
	// per
	...
	// started, it registers its address and related/necessary information with a
	// central
	// repository. For each operation, the PlayersClient/AdministratorClient finds
	// the
	// required information about the associated GameServer from the central
	// repository
	// and invokes the corresponding operation.

	// Output : NA: 6 online, 1 offline. EU: 7 online, 1 offline, AS: 8 online, 1
	// offline.
