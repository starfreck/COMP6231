...
import asianServerServices.AsianServerImplService;
import europeanServerServices.EuropeanServerImplService;
import northAmericanServerServices.NorthAmericanServerImplService;
...
public class PlayerClient {
	...
	public String createPlayerAccount(String FirstName, String LastName, int Age, String Username, String Password,String IPAddress) throws MalformedURLException {
		...
		Object server = getServerObj(serverName);
		
		if(serverName.equals("Asia")) {
			asianServerServices.GameServer serverObj= (asianServerServices.GameServer)server;
			status = serverObj.createPlayerAccount(FirstName, LastName, Age, Username, Password, IPAddress);
		} else if(serverName.equals("Europe")) {
			europeanServerServices.GameServer serverObj= (europeanServerServices.GameServer)server;
			status = serverObj.createPlayerAccount(FirstName, LastName, Age, Username, Password, IPAddress);
		} else if(serverName.equals("NorthAmerica")) {
			northAmericanServerServices.GameServer serverObj= (northAmericanServerServices.GameServer)server;
			status = serverObj.createPlayerAccount(FirstName, LastName, Age, Username, Password, IPAddress);
		}
	...
	public String playerSignIn(String Username, String Password, String IPAddress) throws MalformedURLException{
		...
		if(serverName.equals("Asia")) {
			asianServerServices.GameServer serverObj= (asianServerServices.GameServer)server;
			status = serverObj.playerSignIn(Username, Password, IPAddress);
		} else if(serverName.equals("Europe")) {
			europeanServerServices.GameServer serverObj= (europeanServerServices.GameServer)server;
			status = serverObj.playerSignIn(Username, Password, IPAddress);
		} else if(serverName.equals("NorthAmerica")) {
	...
	public String playerSignOut(String Username, String IPAddress) throws MalformedURLException {
		...
		if(serverName.equals("Asia")) {
			asianServerServices.GameServer serverObj= (asianServerServices.GameServer)server;
			status = serverObj.playerSignOut(Username, IPAddress);
		} else if(serverName.equals("Europe")) {
			europeanServerServices.GameServer serverObj= (europeanServerServices.GameServer)server;
			status = serverObj.playerSignOut(Username, IPAddress);
		} else if(serverName.equals("NorthAmerica")) {
	...
	public String PlayerTransferAccount(String Username, String Password, String oldIPAddress, String newIPAddress) throws MalformedURLException {
		...
		if(serverName.equals("Asia")) {
			asianServerServices.GameServer serverObj= (asianServerServices.GameServer)server;
			status = serverObj.transferAccount(Username, Password, oldIPAddress, newIPAddress);
		} else if(serverName.equals("Europe")) {
			europeanServerServices.GameServer serverObj= (europeanServerServices.GameServer)server;
			status = serverObj.transferAccount(Username, Password, oldIPAddress, newIPAddress);
		} else if(serverName.equals("NorthAmerica")) {
	...
	private static Object  getServerObj(String serverName) throws MalformedURLException {
		...
		if (serverName.equals("Asia")) {
			...
			server = asia.getAsianServerImplPort();
		
		} else if (serverName.equals("Europe")) {
			
			EuropeanServerImplService europe = new EuropeanServerImplService();
			server = europe.getEuropeanServerImplPort();
			
		} else if (serverName.equals("NorthAmerica")) {
			
			NorthAmericanServerImplService northAmerica = new NorthAmericanServerImplService();
			server = northAmerica.getNorthAmericanServerImplPort();
